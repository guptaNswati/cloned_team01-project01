...
public class Physics {
   ...
   public static void shipFlight(Ship ship, Celestial sun, Planet[] planets) {
      if (ship.getOnCelestial()) {
         ...
         double gravityForceOfSun = Constants.GRAVITATIONAL_CONSTANT * sun.getMass()
               / Math.pow(Math.pow(ship.getX() - sun.getX(), 2) + Math.pow(ship.getY() - sun.getX(), 2), 1);
         ...
         for (Planet planet : planets) {
            ...
            double gravityForceOfPlanet = Constants.GRAVITATIONAL_CONSTANT * planet.getMass()
                  / Math.pow(Math.pow(ship.getX() - planet.getX(), 2) + Math.pow(ship.getY() - planet.getX(), 2), 1);
